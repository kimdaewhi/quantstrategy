...
- 변동성 돌파 전략에서의 포지션 비중이 의미하는 바가 무엇인지?
- 제가 지금 일정관리라거나 뭐 그런 전반적인 것들을 하고 있는데, 제가 부득이하게 일정상의 문제가 생기면 도와주실 분이 있으신지?
  없으면 제비뽑기를 할까...흠...생각중...
  

FinanceDataReader.SnapDataReader 함수를 사용을 하면 직전 5개년치의 펀더멘털 주요 데이터를 가져올 수 있다.
이거는 사용하실 분은 참고 하시면 좋을 것 같고
2024년 이후의 데이터, 즉 2025, 2026 데이터는 잘 모르겠음... 제생각엔 컨센서스 추정치같은데, 혹시 FinanceDataReader에 대해서 잘 아시는분은 
공유를 해주시면 좋을 것 같음.
